...
export default function UserDashboard() {
    ...
    const stats = [
        ...
        { label: 'Interviews', value: applications.filter(a => a.status === 'Interview').length, icon: Target, color: 'text-indigo-600' },
        { label: 'Offers', value: applications.filter(a => a.status === 'Accepted').length, icon: Award, color: 'text-emerald-600' },
        { label: 'Success Rate', value: `${applications.length ? Math.round((applications.filter(a => a.status === 'Accepted').length / applications.length) * 100) : 0}%`, icon: TrendingUp, color: 'text-slate-600' },
    ];
                    ...
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                            ...
                            <div className="flex gap-3">
                                <div className="p-1 px-3 bg-white border border-slate-200 rounded-xl flex items-center gap-4 text-xs font-bold text-slate-500">
                                    <span className="flex items-center gap-1.5"><MapPin className="w-3 h-3" /> Paris, FR</span>
                            ...
                            {STATUSES.map((status) => (
                                <div key={status} className="flex flex-col gap-4">
                                    ...
                                    <div className="flex-1 space-y-4">
                                            ...
                                            >
                                                <div className="flex gap-4 mb-5">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-[10px] font-bold text-primary">{app.job.company}</p>
                                                    </div>
                                                </div>
                                        ...
                                        {/* Empty Column State */}
                                        {!loading && applications.filter(a => a.status === status).length === 0 && (
                        ...
                        <div className="mt-16 bg-slate-900 rounded-3xl p-8 relative overflow-hidden">
                            ...
                            <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
                                ...
                                <div className="flex gap-8">
                                    <div>
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Weekly Views</p>
                                        <p className="text-xl font-bold text-white">24</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Response Rate</p>
